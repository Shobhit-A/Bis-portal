...
import { LogOut, CheckCircle, Clock, Save } from 'lucide-react';
...
export default function PortalLayout() {
  ...
  const saveData = useCallback(async (data) => {
    ...
    setSaving(true);
    try {
      await api.put('/submission', { formData: data });
      ...
      <div className="sticky top-0 z-20 shrink-0">
        ...
        <div className="bg-white border-b border-border px-6 py-3 overflow-x-auto scrollbar-hide">
          <div className="flex gap-1 min-w-max">
            {TABS.map((tab, idx) => (
              <button key={tab.key}
                onClick={() => navigate(`/portal/${tab.path}`)}
        ...
        <div className="max-w-4xl mx-auto px-6 py-8">
          ...
          {!location.pathname.includes('submitted') && (
            <div className="flex justify-between mt-8 pt-6 border-t border-border">
              <button onClick={() => { const prev = TABS[currentIndex - 1]; if (prev) navigate(`/portal/${prev.path}`); }}
                disabled={currentIndex === 0} className="btn-secondary disabled:opacity-30">← Previous</button>
              {currentIndex < TABS.length - 1 ? (
                <button onClick={() => { const next = TABS[currentIndex + 1]; navigate(`/portal/${next.path}`); }} className="btn-primary">
                  Next →
                </button>
              ) : null}
            </div>
